...
void rng_configure(int period, char* address, int bit1, int bit2, int bit3, int bit4, int boost_enable)
{
  volatile uint64_t *ptr    = (uint64_t*) IMOC_INST_UPPER;
  volatile uint64_t *ptr1   = ptr + 1;
  volatile uint64_t *ptr2   = ptr + 2;
  uint64_t imo_op           = 0x10;  // Write to control register
  ...
  *(ptr+2) = (uint64_t) 0x1;
  ...
  cr_addr          = 0x4;  // 3rd CR keeps RNG bit indices
  cr_data          = (((uint32_t) bit4) << 16) | (uint16_t) bit3;
  inst_lower       = (cr_addr << 32) | cr_data;
  ...
  *(ptr+2) = (uint64_t) 0x1;
  ...
  cr_addr          = 0x5;  // 4th CR indicates boost enable
  cr_data          = boost_enable;
  inst_lower       = (cr_addr << 32) | cr_data;
...
void copy_row(char *source, char *target)
{
  volatile uint64_t *ptr    = (uint64_t*) IMOC_INST_UPPER;
  uint64_t imo_op           = 0x1;
...
int rng_buf_read()
{
  ...
  *(ptr+2) = (uint64_t) 0x1;
  ...
  volatile uint64_t *data    = (uint64_t*) IMOC_DATA_OFS;
  return (*data) & 0xffffffff; // read a word
}