...
class CacheManagerReq(implicit p: Parameters) extends L1HellaCacheBundle()(p) with HasCoreParameters {
  ...
  when (active) {
    ...
    when (r2_data_req_fired) {
      ...
      when(!r1_data_req_fired) {
        ...
        io.done := ~(data_req_cnt < UInt(refillCycles) || !io.release.ready)
      }
    }
  }
  when (io.req.fire()) {
...
class CacheManager (id: Int) (implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  val io = new Bundle {
    val req         = Decoupled(new CacheManagerReq()).flip // TODO: Flipped
    val wb_req      = Decoupled(new WritebackReq(edge.bundle))
    val meta_write  = Decoupled(new L1MetaWriteReq)
    val grant_valid = Bool(INPUT)
    val wb_done     = Bool(INPUT)
  }
  // 1-) Understand what happens (step-by-step) when a cache miss occurs and 
  //      the cache has to evict a line. This is also called a "voluntary" release
  //      and basically means that the coherency state of the cache line is updated
  //      along with its removal from the cache itself. If the line is dirty, we expect
  //      a writeback.
  // 2-) Specifically, how do we properly trigger a cache line flush?

  val (s_idle :: s_flush :: s_wb_wait :: s_meta_clear :: Nil) = Enum(4)
  val state         = RegInit(s_idle)

  val s0_addr   = io.req.bits.paddr
  val s0_valid  = io.req.valid
  val s0_way    = io.req.bits.way_en

  io.req.ready    := state === s_idle

  val s1_tag  = RegInit(UInt(width = tagBits), 0.U)
  val s1_idx  = RegInit(UInt(width = idxBits), 0.U)
  val s1_way  = RegInit(UInt(width = nWays), 0.U)

  val s1_coh           = Reg(init = ClientMetadata.onReset)

  val (_, s1_shrink_param, s1_clear_coh) = s1_coh.onCacheControl(M_FLUSH)

  io.wb_req.valid           := state === s_flush
  io.wb_req.bits.tag        := s1_tag
  io.wb_req.bits.idx        := s1_idx
  io.wb_req.bits.way_en     := s1_way
  io.wb_req.bits.voluntary  := true.B
  io.wb_req.bits.source     := UInt(id)
  io.wb_req.bits.param      := s1_shrink_param 

  io.meta_write.valid         := state === s_meta_clear
  io.meta_write.bits.idx      := s1_idx
  io.meta_write.bits.data.coh := s1_clear_coh
  io.meta_write.bits.data.tag := s1_tag
  io.meta_write.bits.way_en   := s1_way

  when(state === s_idle)
  {
    s1_tag          := s0_addr >> untagBits
    s1_idx          := s0_addr(untagBits-1, blockOffBits)
    s1_way          := s0_way
    s1_coh          := io.req.bits.meta
    when(s0_valid)
    {
      state := s_flush
      printf("CMON goes to s_flush\n")
    }
  }
  ...
  when(state === s_flush)
  {
    ...
    {
      state := s_wb_wait
      printf("CMON goes to s_wb_wait\n")
    }  
  }

  when(state === s_wb_wait)
  {    
    when(io.wb_req.ready && io.grant_valid)
    {
      state := s_meta_clear
      printf("CMON goes to s_meta_clear\n")
    }
  }

  // clear metadata (dirty bits etc. ?) via meta_write channel
  when(state === s_meta_clear)
  {
    when(io.meta_write.fire())
    {
      state := s_idle
      printf("CMON META CLEAR --- idx:0x%x, way_en:0x%x, tag:0x%x\n",s1_idx, s1_way, s1_tag)
      printf("CMON goes idle\n")
    }
  }
}
...
class ProbeUnit(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  val io = new Bundle {
    ...
    val ongoing_flush = Bool(INPUT)
    val not_sus = Bool(OUTPUT)
  }
  ...
  // This guy should not act up while we are flushing blocks
  io.not_sus := state === s_invalid
  io.req.ready := (state === s_invalid) && !io.ongoing_flush
...
class NonBlockingDCacheModule(outer: NonBlockingDCache) extends HellaCacheModule(outer) {
  ...
  val cman = Module(new CacheManager(0)) // Tilelink thinks this is the first MSHR

  io.cpu.req.ready := Bool(true)
  prober.io.ongoing_flush := Bool(false)
  ...
  when(s1_valid)
  {
    printf("NBD$ - s1_valid_masked:%d s1_req_addr:%x\n", s1_valid_masked, s1_req.addr)
  }
  ...
  val s0_flush_predecode = isFlush(io.cpu.req.bits.cmd)
  ...
  val s1_flush = isFlush(s1_req.cmd)
  val s2_flush = Reg(next = s1_flush, init = Bool(false))


  // Make sure that the prober does not accept new
  // requests while we are trying to flush a CB
  when(s0_flush_predecode && io.cpu.req.ready)
  {
    prober.io.ongoing_flush := Bool(true)
  }
  when(s1_valid && s1_flush)
  {
    prober.io.ongoing_flush := Bool(true)
    io.cpu.req.ready := Bool(false)
  }
  when(s2_valid && s2_flush)
  {
    prober.io.ongoing_flush := Bool(true)
    io.cpu.req.ready := Bool(false)
  }
  ...
  // flushes
  // assume: we do not need to flush if we don't have write permissions
  // also make sure that we don't flush anything if the instr is nack'd
  cman.io.req.valid         := s2_valid_masked && s2_hit && s2_flush
  cman.io.wb_done           := wb.io.done
  when(!cman.io.req.ready) 
  { 
    io.cpu.req.ready := Bool(false)   
    prober.io.ongoing_flush := Bool(true)
  }
  cman.io.req.bits.paddr    := s2_req.addr
  cman.io.req.bits.way_en   := s2_tag_match_way
  cman.io.req.bits.meta     := s2_hit_state
  cman.io.grant_valid       := tl_out.d.fire()

  // print a bunch of interesting signals otherwise I am going to lose my sanity

  printf("s1_valid_masked:%d s1_clk_en:%d s1_flush:%d s1_addr:0x%x s1_tag_eq_way:0x%x s1_tag_match_way:0x%x\n", s1_valid_masked, s1_clk_en, s1_flush, s1_addr, s1_tag_eq_way, s1_tag_match_way)
  printf("s2_valid:%d s2_valid_masked:%d s2_flush:%d s2_tag_match_way:0x%x s2_tag_match:0x%x\n", s2_valid, s2_valid_masked, s2_flush, s2_tag_match_way, s2_tag_match)
  ...
  when(RegNext(s1_clk_en))
  {
    printf("s2_data:0x%x s2_data_muxed:0x%x\n",s2_data.asUInt, s2_data_muxed)
  }
   ...
   * Also we set the 0th id to cache manager, now this code needs to change
   */
  ...
  // flush&invalidation manager
  metaWriteArb.io.in(2) <> cman.io.meta_write

  // writebacks
  val wbArb = Module(new Arbiter(new WritebackReq(edge.bundle), 3))
  wbArb.io.in(0) <> prober.io.wb_req
  wbArb.io.in(1) <> mshrs.io.wb_req
  wbArb.io.in(2) <> cman.io.wb_req
  ...
  // Nack flush only when the address cannot be translated
  val s2_nack_flush = RegEnable(dtlb.io.req.valid && dtlb.io.resp.miss, s1_valid && s1_flush)
  s2_valid_masked := (s2_valid && !s2_nack) || (s2_valid && s2_flush && !s2_nack_flush)
  ...
  cache_resp.valid := ((s2_replay || s2_valid_masked && s2_hit) && !s2_data_correctable) || (s2_flush && s2_valid_masked)

  when(s2_valid)
  {
    printf("NBD$ - s2_valid_masked:%d s2_req_addr:%x s2_hit:%d\n", s2_valid_masked, s2_req.addr, s2_hit)
  }
  ...
  io.cpu.s2_nack := (s2_valid && s2_nack && !s2_flush) || (s2_flush && s2_valid && s2_nack_flush)
  ...
  io.cpu.perf.release := cache_resp.valid && !(s2_flush && s2_valid_masked)
  io.cpu.perf.tlbMiss := io.ptw.req.fire()
  io.cpu.perf.tlbHit  := dtlb.io.req.valid && !dtlb.io.resp.miss && !dtlb.io.req.bits.passthrough

  // Only accept a flush request when all mshrs and the probe unit is idle
  when(s0_flush_predecode && (!mshrs.io.fence_rdy || !prober.io.not_sus || s1_valid || s2_valid || s3_valid || s4_valid || s1_replay || s2_replay))
  {
    io.cpu.req.ready := Bool(false)
  }
}