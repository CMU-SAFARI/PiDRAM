...
uintptr_t sys_alloc_align(int size, int id)
{
  return do_alloc_align(size, id);
}


uintptr_t sys_rcc(uintptr_t src, uintptr_t dest, size_t n)
{
  return do_rcc(src, dest, n);
}

uintptr_t sys_rci(uintptr_t src, size_t n)
{
  return do_rci(src, n);
}

uintptr_t sys_translate(uintptr_t src)
{
  do_translate(src);
  return 0;
}
...
long do_syscall(long a0, long a1, long a2, long a3, long a4, long a5, unsigned long n)
{
  const static void* syscall_table[] = {
    ...
    [SYS_mmap] = sys_mmap,
    [SYS_alloc_align] = sys_alloc_align,
    [SYS_rcc] = sys_rcc,
    [SYS_rci] = sys_rci,
