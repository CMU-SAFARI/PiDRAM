...
zynq_driver_t::zynq_driver_t(tsi_t *tsi, BlockDevice *bdev)
{
    this->tsi = tsi;
    this->bdev = bdev;
    printf("[FESVR]::Initializing Zynq Driver\n");
    ...
    printf("[FESVR]::Trying to reset Zynq System\n");
    sleep(1);
    // reset the target
    write(SYSTEM_RESET, 1);
    write(SYSTEM_RESET, 0);

    printf("[FESVR]::Reset Zynq System\n");
    ...
    printf("[FESVR]::Initialized Zynq Driver\n");
    enable_dump = true;
}
