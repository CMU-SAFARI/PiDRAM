...
class SimAXIMem(channels: Int, forceSize: BigInt = 0)(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    when(io.axi4.head.aw.valid && io.axi4.head.aw.ready)
    {
      printf("aw_addr:0x%x\n",io.axi4.head.aw.bits.addr)
    }

    when(io.axi4.head.w.valid && io.axi4.head.w.ready)
    {
      printf("w_data:0x%x\n",io.axi4.head.w.bits.data)
    }

    when(io.axi4.head.ar.valid && io.axi4.head.ar.ready)
    {
      printf("ar_addr:0x%x\n",io.axi4.head.ar.bits.addr)
    }

    when(io.axi4.head.r.valid && io.axi4.head.r.ready)
    {
      printf("r_data:0x%x\n",io.axi4.head.r.bits.data)
    }
